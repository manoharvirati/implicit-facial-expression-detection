...
CLASSIFIER_PATH = "D:\\projectcv\\cnnVersion2\\projectcnn_face_expression\\projectcnn_face_expression\\support\\lbpcascade_frontalface_improved.xml"
...
def detectFace(image, isColor=True):
    ...
    if os.path.exists(CLASSIFIER_PATH):
        ...
        for i in range(0, len(coordinates)):
            ...
            extraw = int(w * 1 / 10)
            extrah = int(h * 1 / 10)
            if (x - extraw >= 0 and y - extrah >= 0 and x + w + extraw < image.shape[1] and y + h + extrah <
                    image.shape[0]):
